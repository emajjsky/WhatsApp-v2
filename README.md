...
- Windows 客户端当前发布版本为 `0.1.66`
- 后台权限分为普通用户、管理员、超级管理员：普通用户无后台入口；管理员可创建邀请码、配置智能体/Skill/Provider、查看采纳数据及新增普通用户，但用户列表只读；仅 `admin@example.com` 超级管理员可编辑和删除用户。拥有 WhatsApp 账号或采纳日志的用户不可直接删除，以免丢失业务数据；超级管理员账号不可删除或降级。升级时服务端自动执行 `0032_super_admin_role.sql` 迁移，桌面端需要安装新版并重新登录以刷新角色。
- 对话页智能体选择栏改用简短标题和更易点选的下拉框；回复方案、草稿区及导出中心采用无框的 WhatsApp 风格分区，并放大导出表单字号与间距。
- 回复智能体可在后台设置携带上下文的消息条数（默认 20，范围 1–50）；对话页不再显示条数输入，选择区与操作区分行展示，避免窗口缩窄时重叠。
