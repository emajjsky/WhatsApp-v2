...
export DOCKER_IMAGE_PREFIX="${DOCKER_IMAGE_PREFIX:-docker.m.daocloud.io/library/}"

if ! docker version --format '{{.Server.Version}}' >/dev/null 2>&1; then
  echo "Docker daemon is not running. Start Docker Desktop or the Docker service first, then rerun scripts/docker-up.sh." >&2
  exit 1
fi
